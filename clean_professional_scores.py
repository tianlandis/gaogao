...
# -*- coding: utf-8 -*-
"""
山东省2025年音乐类双达线考生专业成绩一分一段表清洗脚本

功能：
1. 读取三个方向的双达线专业成绩分段表（声乐、器乐、音教）
2. 提取专业分和累计人数
3. 追加/合并到 compressed_ranks.json 中

数据来源：
- 山东招生考试院：https://www.sdzk.cn/NewsInfo.aspx?NewsID=6953

使用方法：
    python clean_professional_scores.py

输出文件：
    - compressed_ranks.json (追加专业分位次数据)
"""
...
class ProfessionalScoreCleaner:
    """双达线专业成绩分段表清洗器"""
    ...
    def __init__(self):
        ...
        self.direction_config = {
            'vocal_art_2025': {
                'keywords': ['声乐', '音乐表演-声乐'],
                'files': [
                    '2025年本科艺术统考音乐类（音乐表演-声乐）双达线考生专业成绩一分一段表.xls',
                    '2025年本科艺术统考音乐类（音乐表演-声乐）双达线考生专业成绩一分一段表.xlsx',
                    '2025年本科艺术统考音乐类（音乐表演-声乐）双达线考生专业成绩一分一段表.csv',
                    'vocal_professional.csv'
                ]
            },
            'instrumental_art_2025': {
                'keywords': ['器乐', '音乐表演-器乐'],
                'files': [
                    '2025年本科艺术统考音乐类（音乐表演-器乐）双达线考生专业成绩一分一段表.xls',
                    '2025年本科艺术统考音乐类（音乐表演-器乐）双达线考生专业成绩一分一段表.xlsx',
                    '2025年本科艺术统考音乐类（音乐表演-器乐）双达线考生专业成绩一分一段表.csv',
                    'instrumental_professional.csv'
                ]
            },
            'education_art_2025': {
                'keywords': ['音乐教育', '音教'],
                'files': [
                    '2025年本科艺术统考音乐类（音乐教育）双达线考生专业成绩一分一段表.xls',
                    '2025年本科艺术统考音乐类（音乐教育）双达线考生专业成绩一分一段表.xlsx',
                    '2025年本科艺术统考音乐类（音乐教育）双达线考生专业成绩一分一段表.csv',
                    'education_professional.csv'
                ]
            }
        }
    ...
    def parse_professional_table(self, rows: List[List[str]], direction: str) -> Dict[str, int]:
        """
        解析专业成绩分段表，提取专业分和累计人数
        
        Args:
            rows: CSV/Excel行数据
            direction: 方向标识（vocal_art_2025等）
        ...
        for i, row in enumerate(rows):
            ...
            # 检测表头行（包含"专业成绩"、"分数段"、"累计"等关键词）
            if any(keyword in text for keyword in ['专业成绩', '成绩分数段', '本段人数', '累计人数', '合计']):
                header_found = True
                continue
    ...
    def find_and_process_direction(self, direction_key: str, config: Dict) -> Optional[Dict[str, int]]:
        ...
        Args:
            direction_key: 方向键名（如 vocal_art_2025）
            config: 方向配置
        ...
        for filename in config['files']:
            if os.path.exists(filename):
                ...
                if rows:
                    return self.parse_professional_table(rows, direction_key)
    ...
    def generate_professional_ranks(self) -> Dict:
        """
        生成专业分位次数据
    ...
    def merge_and_save(self, professional_data: Dict):
        """
        合并专业分数据到 compressed_ranks.json
        
        Args:
            professional_data: 新提取的专业分数据
        """
        print("\n" + "="*70)
        ...
        merged_data.update(professional_data)
    ...
    def run(self):
        """执行完整的数据清洗流程"""
        print("\n" + "="*70)
        print("  山东省2025年音乐类双达线专业成绩清洗工具")
        print("  数据来源: https://www.sdzk.cn/NewsInfo.aspx?NewsID=6953")
        print("="*70)
        
        try:
            # 提取专业分数据
            professional_data = self.generate_professional_ranks()
            ...
            # 合并并保存
            self.merge_and_save(professional_data)
            
            # 验证数据完整性
            missing_dirs = [key for key in self.direction_config.keys() if key not in professional_data or not professional_data[key]]
            if missing_dirs:
                print(f"\n⚠️  警告: 以下方向缺少专业分数据:")
...
if __name__ == '__main__':
    cleaner = ProfessionalScoreCleaner()
    cleaner.run()
